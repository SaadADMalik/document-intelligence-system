...
class DocumentPipeline:
    ...
    def _generate_output(self, extracted_docs: List[Dict], search_stats: Dict) -> Dict:
        ...
        successful_docs = sum(1 for d in extracted_docs if not d.get('error'))
        ...
        avg_classification_conf = sum(
            d.get('confidence', 0) for d in extracted_docs if d.get('category') != 'Unclassifiable'
        ) / max(1, total_docs - failed_docs)
