...
class DataExtractor:
    ...
    def __init__(self, offline_mode: bool = True):
        ...
        Args:
            offline_mode: If True, only uses local spacy models
        """
        ...
        try:
            ...
            logger.warning("Continuing without spaCy NER (name extraction will be limited)")
            self.nlp = None
    ...
    def extract(self, text: str, category: str) -> ExtractionResult:
        ...
        """
        # Handle unclassifiable documents gracefully
        if category == "Unclassifiable" or not text or len(text.strip()) < 10:
            return ExtractionResult(
                fields={
                    "error": ExtractedField(value="Document could not be classified", confidence=1.0)
                },
        ...
        if category == "Invoice":
            ...
            return ExtractionResult(
                fields={
                    "note": ExtractedField(value="Generic document - no specific extraction", confidence=1.0)
                },
...
def test_extractor():
    ...
    extractor = DataExtractor(offline_mode=True)
